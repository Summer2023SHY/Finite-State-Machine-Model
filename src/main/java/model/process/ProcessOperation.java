...
//---  Operations   ---------------------------------------------------------------------------
    ...
    public static TransitionSystem buildObserver(TransitionSystem in) {
        ...
        Map<String, List<String>> map = collapseStates(in);
        ...
        while(!queue.isEmpty()) {
            ...
            if(visited.contains(top))
                continue;
            visited.add(top);
            Map<String, List<String>> tran = new HashMap<String, List<String>>();
...
//---  Support Methods   ----------------------------------------------------------------------
    ...
    private static Map<String, List<String>> collapseStates(TransitionSystem in){
    ...
    private static List<String> collapseStatesRecurs(String curr, TransitionSystem in, Map<String, List<String>> map, Set<String> visited){
    ...
    private static TransitionSystem productHelper(TransitionSystem in, TransitionSystem other) {
        ...
        Queue<String> thisNextString = new ArrayDeque<String>();
        Queue<String> otherNextString = new ArrayDeque<String>();
    ...
    private static TransitionSystem parallelCompositionHelper(TransitionSystem in, TransitionSystem other) {
        ...
        Queue<String> thisNextString = new ArrayDeque<>();
        Queue<String> otherNextString = new ArrayDeque<>();
        ...
        while(!thisNextString.isEmpty() && !otherNextString.isEmpty()) { // Go through all the states connected
            ...
            for(String s : events) {
                List<String> inStates = in.getStateEventTransitionStates(stateA, s);
                List<String> otherStates = other.getStateEventTransitionStates(stateB, s);
                if(inStates != null && !inStates.isEmpty() && otherStates != null && !otherStates.isEmpty()) {
                ...
                else if((inStates == null || inStates.isEmpty()) && !in.eventExists(s)) {
    ...
    private static TransitionSystem permissiveUnionHelper(TransitionSystem in, TransitionSystem other) {
        ...
        while(!thisNextString.isEmpty() && !otherNextString.isEmpty()) { // Go through all the states connected
            ...
            for(String s : events) {
                List<String> inStates = in.getStateEventTransitionStates(stateA, s);
                List<String> otherStates = other.getStateEventTransitionStates(stateB, s);
                if(inStates != null && !inStates.isEmpty() && otherStates != null && !otherStates.isEmpty()) {    //if both states have a valid transition
                    for(String t : inStates) {
                        for(String u : otherStates) {
                            compileDestination(t, u, in.getStateComposition(t), other.getStateComposition(u), newString, s, out, use);
                            thisNextString.add(t);
    ...
    private static void compileDestination(String stateA, String stateB, List<String> compA, List<String> compB, String source, String event, TransitionSystem out, List<TransitionSystem> use) {
        List<String> bun = new ArrayList<String>();
        bun.addAll(compA);
        bun.addAll(compB);
    ...
    public static TransitionSystem getSupremalControllableSublanguage(TransitionSystem other) {
        ...
        Map<String, DisabledEvents> disabledMap = new HashMap<String, DisabledEvents>();
        // Parse the graph and identify disabled states and disabled events
        for(String s : states.getStrings()) {
            Set<String> visitedStrings = new HashSet<String>();
        ...
        List<String> statesToRemove = new ArrayList<String>();
        ...
        for(String s : newTransitionSystem.getStrings()) {
            ...
            if(disabled.stateIsDisabled()) {
                ...
                List<Transition> transitions = this.transitions.getTransitions(getString(s));
                if(transitions != null) {
                    for(Transition t : transitions) {
                        ...
                        if(disabled.eventIsEnabled(e.getEventName())) {
                            ...
                            for(String toS : (List<String>)t.getTransitionStrings()) {
...
//        System.out.println(disabledMap.toString());
    ...
    public static ModalSpecification makeOptimalSupervisor(TransitionSystem ModalSpecification) throws IllegalArgumentException {
        ...
        Map<String, String> universalObserverViewMap = createUniversalObserverView(ModalSpecification, universalObserverView);
     ...
     * @param badStrings - Map of all the states already marked as bad, which will be further updated as
     * we go through this iteration.
     * @return - Returns true if at least one state was marked as bad, false otherwise.

    private boolean markBadStrings(TransitionSystem fsm, TransitionSystem specTransitionSystem, TransitionSystem product, Set<String> badStrings) {
        ...
        for(String s : product.getStrings()) {
            ...
            List<Transition> specTransitions = this.mustTransitions.getTransitions(this.getString(specStringName));
            if(specTransitions != null) for(Transition t : specTransitions) {
                String event = t.getTransitionEvent();
                List<String> toStrings = product.transitions.getTransitionStrings(s, product.events.getEvent(event));
            ...
            List<String> origStrings = product.getStringComposition(observerString);
            for(String fromString : origStrings) {
                // Go through all the original transitions
                List<Transition> origTransitions = fsm.transitions.getTransitions((String)fromString);
                if(origTransitions != null) {
                    for(Transition t : origTransitions) {
                        ...
                        if(event.getEventObservability() &&  !event.getEventControllability()) {
                            ...
                            List<String> toStrings = product.transitions.getTransitionStrings(product.getString(s), product.events.getEvent(event));
        ...
        return foundABadOne;
    } // markBadStrings(TransitionSystem, TransitionSystem, TransitionSystem, Set)
     ...
     * @param badStrings - Set of String state names which are bad and to be removed.
     * @return - Returns a boolean value; true if the method marked a bad state, false otherwise.

    public static boolean markDeadEnds(TransitionSystem universalObserverView, Map<String, String> universalObserverViewMap, TransitionSystem product, Set<String> badStrings) {
        ...
        List<String> productInitialStrings = product.getInitialStrings();
        ...
        for(String productString : product.getStrings()) if(!badStrings.contains(productString.getStringName())) {
            ...
            List<String> originalStrings = product.getStringComposition(leftSideOfProduct);
    ...
    } // markDeadEnds(TransitionSystem, Map, TransitionSystem, Set)
    ...
    static private boolean canReachMarked(TransitionSystem fsm, String state, Set<String> badStrings) {
        ...
        return false;
    } // canReachMarked(TransitionSystem, Set)
    */
